<?php
	if (isset($_POST["reg_id"])) {
		...
		if ($result = $mysqli->query("SELECT id name FROM user where id='{$id}' or name='{$name}';")) {
			if($result->num_rows!==0){
				$error .= '<p class="error">IDまたはニックネームが既に登録済みです</p>';
			}
		...
		if ($error == "" ) {
			...
			if ($mysqli->query("insert into user (id,name,pass,points,team) values('{$id}','{$name}','{$passd}','0','{$team}');")){
...
<form role="form" method=post>
  ...
  <div class="form-group">
    ...
    <input type="text" name=reg_name class="form-control" id="name" value="<?php echo $name?>" placeholder="Enter nickname">
  </div>
  <div class="form-group">
